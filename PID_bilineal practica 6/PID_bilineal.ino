...
void loop() {
  ...
  distance = (sonar.ping() / 29.287)*10; // microseg a mm
  distance = 190; //ajusto el cero
...
float PID_bilineal(float ref, float medicion){
  ...
  float kp = 0, kd = 0, ki = 0, Ts = 0.02;
  float uk = 0;
  volatile float ek_1 = 0, ek = 0, Dk = 0, Ik = 0, IK = 0;

  // Cálculo de error
  ek = ref - (medicion);

  // Término integral (usando integración trapezoidal)
  IK = Ik + (Ts/2) * ek + (Ts/2) * ek_1;
